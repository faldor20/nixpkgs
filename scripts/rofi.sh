...
## This script is based on Zach Baylin's  WiFi Menu.
## https://github.com/zbaylin/wofi-wifi-menu
...
## Dynamically change the size of the Wofi menu, with a cap on network count.
## Wofi seems to require some text padding dependent on our font size.
...
CHENTRY=$(echo -e "$MENU" | uniq -u | wofi -dmenu -p "WiFi SSID" -lines "$LINE_COUNT" -a "$HIGHLINE" -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH")
CHSSID=$(echo "$CHENTRY" | sed  's/\s\{2,\}/\|/g' | awk -F "|" '{print $1}')

## Support manual SSID entry.
if [ "$CHENTRY" = "manually connect to a network" ] ; then
	MSSID=$(echo "Enter your network's SSID." | wofi -dmenu -p "SSID: " -font "$FONT" -lines 1)
	WIFI_PASS=$(echo "Enter the network password." | wofi -dmenu -password -p "Password: " -lines 1 -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH")
